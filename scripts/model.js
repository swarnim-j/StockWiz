...
function makePredictions(X, model, dict_normalize) {
  ...
  //console.log("X after tensor operation:");
  //const temp=(tf.tensor2d(X, [X.length, X[0].length]).div(tf.scalar(10))).mul(10);
  //console.log(temp);
  //alert("hey");
  //console.log(Array.from(temp.dataSync()));
...
function normalizeTensorFit(tensor) {
  ...
  const minval = tensor.min();   //these are being displayed as empty
